...
session_start();
$hid = $_SESSION['hid'];
$sql1 = mysqli_query($con," SELECT `patient_id` as pid, `patient_name` as pn, `patient_phone_number` as ppn FROM `patient_details`");
?>
								...
								<div class="card-body">
									<div class="table-responsive">
										<table id= "myTable" class="table table-striped">
											...
											<tbody>
												<?php
                                                    while($run = mysqli_fetch_assoc($sql1))
                                                    {
                                                        $p = $run['pid'];
                                                        echo '<tr>
                                                        <td>'.$run['pid'].'</td>
                                                        <td>'.$run['pn'].'</td>
                                                        <td>'.$run['ppn'].'</td>';
														$sss = mysqli_query($con,"select count(*) as c from access_rights where patient_id='$p' and hid='$hid'");
                                                        $ans = mysqli_fetch_assoc($sss);
                                                        if($ans['c'] == 0)
                                                        {
                                                            echo '<td><a href="getaccess.php?pid='.$p.'">
                                                           GET ACCESS
                                                            </a></td>';
                                                        }
                                                        else
